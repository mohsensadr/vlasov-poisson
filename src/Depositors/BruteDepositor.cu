...
/*
__global__ void deposit_velocity_2d(float_type *x, float_type *y, float_type *N, float_type *vx, float_type *vy, float_type *Ux, float_type *Uy, int n_particles,
            int N_GRID_X, int N_GRID_Y,
            float_type Lx, float_type Ly
    ) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
/*
__global__ void deposit_temperature_2d(float_type *x, float_type *y, float_type *N, float_type *vx, float_type *vy, float_type *Ux, float_type *Uy, float_type *T, int n_particles,
            int N_GRID_X, int N_GRID_Y,
            float_type Lx, float_type Ly
    ) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n_particles) {
        ...
        int iy = int(y[i] / Ly * N_GRID_Y) % N_GRID_Y;
        int idx = ix + iy * N_GRID_X;
        float_type energy = (vx[i]-Ux[idx])*(vx[i]-Ux[idx]);
